...
class ServicesRepository {
  ...
  Future getSavingsBalanceDisbursementRequestDetailService() async {
    var response = await HTTPClientContract.instance.getHTTP('/special/getIdl_HL_Detail?secNo=9661010192');
    if (kDebugMode) {
      print(response);
    }
    if (response != null && response.statusCode == 200) {
...
/// **************************************************************MATERNITY ALLOWANCE APPLICATION  - START******************************************************
  ...
  Future setMaternityAllowanceApplicationService(result, docs, paymentInfo, newBornData, birthExpectation, childSerialNo, childNatNo, childNationality, birthPlace, leaveStartDate, leaveEndDate, birthDate) async {
    var row = {
      ...
      "OTHER_DEPENDANTS": null,
      "ID": "",
      "LEAVE_START_DATE": DateFormat('dd/MM/yyyy').format(leaveStartDate),
      "LEAVE_END_DATE": DateFormat('dd/MM/yyyy').format(leaveEndDate),
