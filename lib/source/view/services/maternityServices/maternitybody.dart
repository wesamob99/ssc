...
class MaternityBody extends StatefulWidget {
  const MaternityBody({Key key}) : super(key: key);

  @override
  State<MaternityBody> createState() => _MaternityBodyState();
}

class _MaternityBodyState extends State<MaternityBody> {

  List<Service> maternityServices = ServicesList.maternityServices;
  List supTitles = [];
  @override
  void initState() {
    for (var element in maternityServices) {
    ...
    return Expanded(
      child: ListView.builder(
          ...
          itemBuilder: (context, index1){
            int last = -1;
            last = maternityServices.lastIndexWhere((element) => element.supTitle == supTitles[index1]);
            return Column(
              ...
              children: [
                ...
                ListView.builder(
                  ...
                  itemCount: maternityServices.length,
                  itemBuilder: (context, index2){
                    return maternityServices[index2].supTitle == supTitles[index1]
                        ? Column(
                      children: [
                        InkWell(
                          onTap: (){
                            Navigator.of(context).push(
                              MaterialPageRoute(
                                  builder: (context) => AboutTheServiceScreen(
                                      serviceScreen: maternityServices[index2].screen,
                                      serviceTitle: maternityServices[index2].title,
                                      aboutServiceDescription: maternityServices[index2].description,
                                      ...
                                      serviceApiCall: maternityServices[index2].serviceApiCall
                                  )
                              ),
                            );
                          },
                          child: Container(
                            ...
                            child: Text(
                              getTranslated(maternityServices[index2].title, context),
                              style: TextStyle(
